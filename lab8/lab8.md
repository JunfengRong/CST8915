...
## Demo Video
https://www.youtube.com/watch?v=ssG32ZDyguQ



## Azure Managed Services to Replace MongoDB and RabbitMQ
### MongoDB → Azure Cosmos DB (MongoDB API)

Service： Azure Cosmos DB

Why it's a good fit:
- Fully managed NoSQL database with global distribution and multi-region replication.
- Compatible with MongoDB drivers and tools via the MongoDB API.
- Automatic scaling, built-in backup, and high availability (99.999% SLA).
- Eliminates the need to manage Replica Sets, PVCs, or StatefulSets manually.
...
Why it's a good fit:
- Fully managed enterprise-grade messaging service supporting queues and topics (Pub/Sub).
- Supports AMQP protocol, making it compatible with RabbitMQ clients.
- Built-in message durability, dead-lettering, and retry policies.
- Scales automatically and integrates with Azure Functions, Logic Apps, and Event Grid.

### yaml file (including task 1 and task 2)
```yaml
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: order-service
          image: rong0010/order-service-l8:v2 # Custom image for order service
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: makeline-service
          image: rong0010/makeline-service-l8:v2 # Custom Makeline service image
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: product-service
          image: rong0010/product-service-l8:v2 # Custom product service image
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: store-front
          image: rong0010/store-front-l8:v2 # Front-end store application
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: store-admin
          image: rong0010/store-admin-l8:v2 # Admin interface for store
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: ai-service
          image: rong0010/ai-service-l8:v2
          ports:
            - containerPort: 5001
          env:
